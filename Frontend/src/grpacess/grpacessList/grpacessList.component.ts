import { Component, ViewChild, Inject } from '@angular/core';
import { MatDialog, MAT_DIALOG_DATA } from '@angular/material/dialog';
import { MatPaginator } from '@angular/material/paginator';
import { MatSnackBar } from '@angular/material/snack-bar';
import { MatTableDataSource } from '@angular/material/table';
import { PaginationType } from 'src/api/models/pagination';
import { GrpAcess } from 'src/api/models/grpacess';
import { GrpAcessService } from 'src/api/services/grpacess.services';

@Component({
  selector: 'grpacessList-root',
  templateUrl: './grpacessList.component.html',
  styleUrls: ['./grpacessList.component.css'],
})
export class GrpAcessListComponent {
  displayedColumns: string[] = ['id', 'name', 'action'];
  grpacessList: GrpAcess[] = [];
  pagination: PaginationType = {
    length: 0,
    size: 0,
    lastPage: 0,
    page: 0,
    startIndex: 0,
    endIndex: 0,
  };
  dataSource: MatTableDataSource<GrpAcess>;

  page = 0;
  size = 25;
  search = '';
  order = 'asc';
  sort = 'id';

  @ViewChild(MatPaginator, { static: true }) paginator: MatPaginator;

  constructor(
    private grpacessService: GrpAcessService,
    private dialog: MatDialog,
    private snackBar: MatSnackBar
  ) {}

  ngOnInit() {
    this.paginator._intl.itemsPerPageLabel = 'Itens por página:';
    this.paginator._intl.nextPageLabel = 'Próxima';
    this.paginator._intl.previousPageLabel = 'Anterior';
    this.paginator._intl.firstPageLabel = 'Primeira página';
    this.paginator._intl.lastPageLabel = 'Última página';
    this.paginator._intl.getRangeLabel = this.getDisplayText;

    this.grpacessService.getUnityPage().subscribe({
      next: (resp) => {
        this.grpacessList = resp.results;
        this.pagination = resp.pagination;
        this.dataSourceValue();
      },
      error: (error) => {
        if (error.error.message) {
          this.snackBar.open(`${error.error.message}`, 'Error', {
            duration: 3000,
          });
        } else {
          this.snackBar.open(`Ocorreu um erro inesperado.`, 'Error', {
            duration: 3000,
          });
        }
      },
    });
  }

  getDisplayText(page: number, pageSize: number, length: number) {
    if (length == 0 || pageSize == 0) {
      return 'Sem resultados.';
    }
    const startIndex = page * pageSize;
    const endIndex =
      startIndex < length
        ? Math.min(startIndex + pageSize, length)
        : startIndex + pageSize;
    return `Itens: ${startIndex + 1} - ${endIndex} Página: ${
      page + 1
    } de ${Math.ceil(length / pageSize)}, total:${length} itens`;
  }

  dataSourceValue() {
    this.dataSource = new MatTableDataSource<GrpAcess>(this.grpacessList);
  }

  handlePageEvent(event: any) {
    this.grpacessService
      .getUnityPage(
        event.pageIndex,
        this.size,
        this.search,
        this.order,
        this.sort
      )
      .subscribe({
        next: (resp) => {
          this.grpacessList = resp.results;
          this.pagination = resp.pagination;
          this.dataSourceValue();
        },
        error: (error) => {
          if (error.error.message) {
            this.snackBar.open(`${error.error.message}`, 'Error', {
              duration: 3000,
            });
          } else {
            this.snackBar.open(`Ocorreu um erro inesperado.`, 'Error', {
              duration: 3000,
            });
          }
        },
      });
  }

  onChangeInput(e: any) {
    this.search = e.target.value;

    this.grpacessService
      .getUnityPage(0, this.size, this.search, this.order, this.sort)
      .subscribe({
        next: (resp) => {
          this.grpacessList = resp.results;
          this.pagination = resp.pagination;
          this.dataSourceValue();
        },
        error: (error) => {
          if (error.error.message) {
            this.snackBar.open(`${error.error.message}`, 'Error', {
              duration: 3000,
            });
          } else {
            this.snackBar.open(`Ocorreu um erro inesperado.`, 'Error', {
              duration: 3000,
            });
          }
        },
      });
  }

  openDialog(e) {
    const dialog = this.dialog.open(DialogRemove, {
      data: { id: e.id, nome: e.nomegrp },
    });
    dialog.afterClosed().subscribe((remove) => {
      if (remove) {
        this.removeUnity(e.id);
      }
    });
  }

  removeUnity(unityId) {
    this.grpacessService.remove(unityId).subscribe({
      next: (_) => {
        this.grpacessList = this.grpacessList.filter((item) => item.codgrp != unityId);
        this.pagination.length = this.pagination.length - 1;
        this.dataSourceValue();
        this.snackBar.open('Removido com sucesso.', 'Sucesso', {
          duration: 3000,
        });
      },
      error: (error) => {
        if (error.error.message) {
          this.snackBar.open(error.error.message, 'Error', {
            duration: 50000,
          });
        } else {
          this.snackBar.open('Ocorreu um erro inesperado.', 'Error', {
            duration: 3000,
          });
        }
      },
    });
  }
}

@Component({
  selector: 'dialog-remove',
  template: ` <div class="dialog-container">
    <div class="dialog-container-title">
      <h1 mat-dialog-title class="dialog-title">Remover unidade</h1>
    </div>
    <div mat-dialog-content class="dialog-message">
      Gostaria de remover unidade de nome: {{ data.nomegrp }} com id:
      {{ data.codgrp }}?
    </div>
    <div mat-dialog-actions>
      <button mat-button [mat-dialog-close]="false">Cancelar</button>
      <button mat-button [mat-dialog-close]="true" cdkFocusInitial color="warn">
        Remover
      </button>
    </div>
  </div>`,
  styleUrls: ['./grpacessList.component.css'],
})
export class DialogRemove {
  constructor(@Inject(MAT_DIALOG_DATA) public data: GrpAcess) {}
}
