...
export interface Atendimento {  
  ...
  atendServ?: { // Adiciona a propriedade atendServ
    Obs: string;
    CodServ: number;
    CodProf: number;
    DtAgen: string;
...
export class AtendimentoService {  
  ...
  deleteServicos(codAtend: number, servicos: Servico[]): Observable<void>[] {  
    return servicos.map((servico) => this.deleteServico(codAtend, servico.IdServico)); // Substitua pelo nome correto da propriedade
  }  
  ...
  // Obter serviços associados a um atendimento
  getServicosByAtendimento(codAtend: number): Observable<Servico[]> {
    const headers = this.getHeaders();
    return this.http.get<Servico[]>(`${environment.baseUrl}${this.PATH}/${codAtend}/servicos`, { headers });
  }
